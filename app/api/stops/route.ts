...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !coordinates || coordinates.length !== 2) {
      return NextResponse.json({ error: "Name and valid coordinates are required" }, { status: 400 })
    }
    ...
    if (existingStop) {
      return NextResponse.json({ error: "Stop with this name already exists" }, { status: 400 })
    }
