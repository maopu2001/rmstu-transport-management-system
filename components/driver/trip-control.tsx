...
import { MapPin, Clock, Users, Navigation, AlertTriangle, CheckCircle } from "lucide-react"
...
interface ActiveTrip {
  ...
  passengers: number
  status: "On Schedule" | "Delayed - Traffic" | "Delayed - Breakdown" | "Delayed - Other"
}
...
export function TripControl() {
  const [activeTrip, setActiveTrip] = useState<ActiveTrip | null>(mockActiveTrip)
  ...
  useEffect(() => {
    if (activeTrip && isGPSActive) {
      const locationInterval = setInterval(() => {
        const mockLocation = {
          lat: 22.4569 + (Math.random() - 0.5) * 0.001,
          lng: 91.9677 + (Math.random() - 0.5) * 0.001,
        }
        updateLocation(activeTrip.vehicleNumber, mockLocation, activeTrip.status)
      }, 5000)
  ...
  const handleStatusChange = (newStatus: ActiveTrip["status"]) => {
    if (activeTrip) {
      ...
      updateLocation(activeTrip.vehicleNumber, mockLocation, newStatus)
    }
  }
  ...
  const progress = activeTrip ? (activeTrip.currentStop / activeTrip.totalStops) * 100 : 0
      ...
      <div className="space-y-6">
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-gray-600">Start a trip from your schedule to begin tracking</p>
          </CardContent>
        </Card>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge variant={getStatusBadgeVariant(activeTrip.status)}>{activeTrip.status}</Badge>
          </div>
        </CardHeader>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <div className={`w-3 h-3 rounded-full ${isGPSActive ? "bg-green-500" : "bg-red-500"}`} />
              <span className="font-medium">GPS Tracking</span>
            </div>
            <Badge variant={isGPSActive ? "default" : "destructive"}>{isGPSActive ? "Active" : "Inactive"}</Badge>
          </div>
          <p className="text-sm text-gray-600 mt-2">
            {isGPSActive ? "Your location is being broadcast to students and admin" : "Location tracking is disabled"}
          </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Let students and admin know your current status</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track your progress through the route</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {activeTrip.stops.map((stop, index) => (
              <div key={index} className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  <div className={`font-medium ${stop.reached ? "text-green-600" : "text-gray-900"}`}>{stop.name}</div>
                  <div className="text-sm text-gray-500">{stop.estimatedTime}</div>
                </div>
                {stop.reached && <CheckCircle className="h-5 w-5 text-green-600" />}
                {index === activeTrip.currentStop && <Navigation className="h-5 w-5 text-primary" />}
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="space-y-3">
        ...
        <Button onClick={handleEndTrip} variant="outline" className="w-full bg-transparent" size="lg">
          End Trip
        </Button>
      </div>
    </div>
